...
class WalletWithdrawExport implements FromQuery, WithMapping, WithHeadings
{
    ...
    public function headings() : array
    {
        ...
        } elseif ($this->originType == WalletWithdraw::ORIGIN_TYPE_CS){
            $array = [
                '提现时间',
                '提现编号',
                '提现金额',
    ...
    public function map($row) : array
    {
        ...
        } elseif ($row->origin_type == WalletWithdraw::ORIGIN_TYPE_CS) {
            $row->merchant_name = $row->csMerchant->name;
            $row->oper_name = OperService::getNameById($row->csMerchant->oper_id);
            // 发票信息
            if ($row->bank_card_type == WalletWithdraw::BANK_CARD_TYPE_PEOPLE) {
                $invoiceExpress = '无需发票';
            } elseif ($row->bank_card_type == WalletWithdraw::BANK_CARD_TYPE_COMPANY) {
        ...
        if ($this->originType == WalletWithdraw::ORIGIN_TYPE_MERCHANT){
            $array = [
                ...
                $row->bank_card_no,
                $row->bank_name,
                $row->merchant->sub_bank_name,
                $row->merchant->bank_province.$row->merchant->bank_city.$row->merchant->bank_area.$row->merchant->bank_open_address,
            ];
        } elseif ($this->originType == WalletWithdraw::ORIGIN_TYPE_USER) {
            $array = [
                $row->created_at,
            ...
            $array = [
                ...
                $row->bank_card_no,
                $row->bank_name,
                $row->oper->sub_bank_name,
                $row->oper->bank_open_address,
            ];
        } elseif ($this->originType == WalletWithdraw::ORIGIN_TYPE_BIZER) {
            $array = [
                $row->created_at,
            ...
            $array = [
                ...
                $row->bank_card_no,
                $row->bank_name,
                $row->csMerchant->sub_bank_name,
                $row->csMerchant->bank_province.$row->csMerchant->bank_city.$row->csMerchant->bank_area.$row->csMerchant->bank_open_address,
            ];
        } else {
            // 批次明细 导出
            $array = [
