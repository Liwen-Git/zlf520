...
class CsMerchantController extends Controller
{
    ...
    public function getList()
    {
        ...
        if($operName) {
            $operIds = Oper::where('name','like',"%{$operName}%")->pluck('id')->toArray();
            if (empty($operId)) {
                $operId = !empty($operIds) ? $operIds:-1;
            }
        }
    ...
    public function edit()
    {
        ...
        $merchant = CsMerchantService::edit(request('id'));
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function getCsMerchantName()
    {
        $this->validate(request(), [
            'keyword' => 'required',
        ]);
        $keyword = request('keyword');
        $list = CsMerchantService::getAllNames(['name' => $keyword]);
    ...
    public function audit()
    {
        ...
        $supermarket = CsMerchant::findOrFail($id);

        switch ($type){
            case '1': // 审核通过
                $supermarket->audit_status = CsMerchant::AUDIT_STATUS_SUCCESS;
                $supermarket->status = CsMerchant::STATUS_ON;
                $supermarket->audit_suggestion = $auditSuggestion;
                $supermarket->save();
                break;
            case '2': // 审核不通过
                $supermarket->audit_status = CsMerchant::AUDIT_STATUS_FAIL;
                $supermarket->status = CsMerchant::STATUS_OFF;
                $supermarket->audit_suggestion = $auditSuggestion;
                $supermarket->save();
                break;
            default:
