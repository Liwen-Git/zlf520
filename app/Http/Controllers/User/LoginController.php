        ...
        if($is_lock){
            ...
            try {
                ...
                if ($isFirstSign == 1) {
                    ...
                    if( $shareUserId && empty( InviteUserRecord::where('user_id', $user->id)->first() ) ) {
                        $originId = $shareUserId;
                        $originType = InviteChannel::ORIGIN_TYPE_USER;

                        $inviteChannel = InviteChannelService::getByOriginInfo($originId, $originType);
                        InviteUserService::bindInviter($user->id, $inviteChannel);
                    }
                }
