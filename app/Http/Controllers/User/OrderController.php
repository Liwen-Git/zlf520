    ...
    public function detail()
    {
        ...
        if ($detail->type == Order::TYPE_DISHES) {
            ...
            if ($csItems->isNotEmpty()) {
                foreach ($csItems as &$item) {
                    ...
                    if($detail->pintuan_order_id!=0) {
                        $detail->goods_detail = CsGood::find($item->cs_goods_id);
                    }
                }
            }
            ...
            if (!empty($detail->marketing_id)) {
                $amount_ori = 0;
                $amount = 0;
                foreach ($detail->cs_items as $k=>$v) {
    ...
    public function buy()
    {
        ...
        try {
            ...
            if(!empty($canDiscountGoodsList)){
                ...
                $limitedNum = ($discountNumber<$canDiscountGoodsList['limited']) ? $discountNumber:$canDiscountGoodsList['limited'];
                $limitedNum = ($number<$limitedNum) ? $number : $limitedNum;
...
//            throw new BaseResponseException('系统错误，创建订单失败');
    ...
    public function csOrderCreate(){
        ...
        foreach ($phaseGoodsList as $item) {
            if(!isset($item) || $item['number'] <= 0){
                throw new ParamInvalidException('活动商品可购买数量必须大于零');
            }
        }
        ...
        $goodList = OrderService::getCsOrderGoodsList($merchant, $goodsList);
        $canDiscountGoodsList = OrderService::getCanDiscountGoodsList(
            $phaseId,
            $marketingId,
            $phaseGoodsIds,
            $merchantId,
            $user->id,
            OrderService::MARKETING_GOODS_TYPE_SUPERMARKET,
            $clientGoodNNumberList);           // 可优惠商品
        $goodsPrice = empty($canDiscountGoodsList) ?
            OrderService::getCsOrderGoodsPrice($goodList) :
            OrderService::computedDiscountMoneyForPhase($canDiscountGoodsList,$goodList,$clientGoodNNumberList);
        ...
        // 超市订单 下单判断是否添加分享人
        $shareUserId = 0;
        if (request('share_user_id') && request('share_key') && request('share_token')) {
            $shareKey = request('share_key');
            $shareToken = request('share_token');
            if ($shareToken == Cache::get($shareKey)) {
        ...
        try {
            ...
            $order->ybk_anchor_id = $ybk_anchor_id;
            $order->share_user_id = $shareUserId;
            if ($order->save()) {
