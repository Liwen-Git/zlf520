...
class OrderController extends Controller
{
    ...
    public function getList()
    {
        ...
        $data = Order::where('user_id', $user->id)
            ...
            ->where('pintuan_order_id', 0)     // 不看拼团单
...
//            $item->isOperSelf = $item->oper_id === $currentOperId ? 1 : 0;
    ...
    public function detail()
    {
        ...
        $merchant_of_order = ($detail->merchant_type == Order::MERCHANT_TYPE_SUPERMARKET) ? CsMerchantService::getById($detail->merchant_id):MerchantService::getById($detail->merchant_id);
        ...
        if ($detail->status == Order::STATUS_FINISHED && $detail->splitting_status == Order::SPLITTING_STATUS_NO){
            if( (request()->header('version') >= '1.5.4')){
                $detail->fee_splitting_amount .= ' (正在结算中...)';
            }
        } else {
            ...
            if(!empty($feeSplittingRecords)){
                ...
                if ($detail->merchant_type == Order::MERCHANT_TYPE_SUPERMARKET) {
                    $ratio = FeeSplittingService::userFeeSplittingRatioOfCs($detail->settlement_rate);
                } else {
                    $ratio = FeeSplittingService::UserFeeSplittingRatio($detail->settlement_rate);
                }
                if ($ratio < 0) $ratio = 0;
                $detail->fee_splitting_amount = Utils::floorDecimal($detail->pay_price * $ratio);
            }
            $detail->fee_splitting_amount = '¥' . $detail->fee_splitting_amount;
        }
        //商家详情
        ...
        if ($detail->type == Order::TYPE_DISHES) {
            ...
            $detail->good = Goods::withTrashed()->where('id', $detail->goods_id)->first();
        ...
        // 单品订单
        $detail->marketing_discount = 0;//优惠金额
        if ($detail->type == Order::TYPE_DISHES) {
            ...
            if (!empty($detail->marketing_id)) {
                $amount_ori = 0;
                $amount = 0;
                foreach ($detail->dishes_items as $k=>$v) {
                    $amount_ori += $v->ori_price * $v->number;
                    $amount += $v->dishes_goods_sale_price * $v->number;
                }
                $detail->marketing_discount = $amount_ori - $amount;
                if ($detail->marketing_discount<0) {
                    $detail->marketing_discount = 0;
                }
            }
            $goods_list = [];
            foreach ($detail->dishes_items as $k=>$v) {
                if (!empty($goods_list[$v->dishes_goods_id])) {
                    $goods_list[$v->dishes_goods_id]['number'] += $v->number;
                } else {
                    $goods_list[$v->dishes_goods_id] = [
                        'goods_id'=>$v->dishes_goods_id,
                        'goods_name'=>$v->dishes_goods_name,
                        'img_url'=>$v->dishes_goods_detail_image,
                        'number'=>$v->number,
                        'ori_price'=>$v->ori_price
                    ];
                }
            }
            $detail->goods_list = collect();
            foreach ($goods_list as $v) {
                $v = (object) $v;
                $detail->goods_list->push($v);
            }
        }else if($detail->type == Order::TYPE_GROUP_BUY){
            $detail->goods_detail = Goods::withTrashed()->find($detail->goods_id);
            $detail->goods_end_date = $detail->goods_detail->end_date;
            if (!empty($detail->marketing_id)) {
                $detail->marketing_discount = $detail->original_price - $detail->pay_price;
                if ($detail->marketing_discount<0) {
                    $detail->marketing_discount = 0;
                }
            }
            ...
            if (!empty($detail->marketing_id)) {
                ...
                foreach ($detail->order_goods as $k=>$v) {
                    $amount_ori += $v->original_price * $v->number;
                    $amount += $v->price * $v->number;
                }
                $detail->marketing_discount = $amount_ori - $amount;
                if ($detail->marketing_discount<0) {
                    $detail->marketing_discount = 0;
            ...
            foreach ($detail->order_goods as $k=>$v) {
                if (!empty($goods_list[$v->dishes_goods_id])) {
                    ...
                    $goods_list[$v->dishes_goods_id] = [
                        'goods_id'=>$v->cs_goods_id,
                        'goods_name'=>$v->goods_name,
                        'img_url'=>$v->cs_goods->logo,
                        'number'=>$v->number,
                        'ori_price'=>$v->original_price
                    ];
                }
                if($detail->pintuan_order_id!=0) {
                    $detail->goods_detail = CsGood::find($v->cs_goods_id);
                }
            }
            $detail->goods_list = collect();
            foreach ($goods_list as $v) {
        ...
        $detail->marketing_discount = round($detail->marketing_discount,2);
        $detail->marketing_discount = sprintf("%.2f",$detail->marketing_discount);
...
//                $detail->merchant_service_phone = $csMerchant->service_phone;
        ...
        if (!empty($detail->ybk_anchor_id)) {
            ...
            if($anchor) $detail->ybk_anchor_nickname = $anchor->nickname;
        }
        if($detail->pintuan_order_id!=0){
            $goodsType = [
                Order::TYPE_GROUP_BUY => 1,
                Order::TYPE_SUPERMARKET => 3,
            ];
            GoodsService::insertGroupBookDetail($detail->goods_detail, $goodsType[$detail->type]);
            $groupBookOrder = MarketingApi::getGroupBookOrderById($detail->pintuan_order_id);
            $groupBookOrderTitle = [
                ...
                GroupBookController::ORDER_STATUS_OPEN_FAIL => '开团失败',
            ];
            $groupBookOrder['title'] = $groupBookOrderTitle[$groupBookOrder['status']];
            $detail->group_book_order = $groupBookOrder;
            $detail->is_group_booking = 1;
            $marketingGoodsDetail = MarketingApi::getMarketingGoodsById($detail->marketing_goods_id);
            // 添加拼团商品信息
            if ($marketingGoodsDetail) {
                $detail->marketing_goods_datil = $marketingGoodsDetail;
            }
        }
    ...
    public function buy()
    {
        ...
        $phaseId = request('phase_id', 0);
        $marketingId = request('marketing_id');
        $discountNumber = request('discount_number');
        $user = request()->get('current_user');
        $groupBookNumber = request('group_book_number');
        $groupBookTeamId = request('group_book_team_id',0);

        $goods = Goods::findOrFail($goodsId);
        if ($goods->stock < $number) {
            throw new BaseResponseException('商品库存不足',ResultCode::STOCK_EMPTY);
        }

        $merchant = MerchantService::getById($goods->merchant_id);
        $this->checkMerchant($merchant);

        $canDiscountGoodsList = OrderService::getCanDiscountGoodsListForGroup($phaseId,$marketingId,$discountNumber,$merchant->id,$user->id,OrderService::MARKETING_GOODS_TYPE_GROUP,$goods->id,$discountNumber);
        ...
        //通过云博客主播编号查询主播ID
        $ybk_anchor_id = request('anchor_id','');

        if (!empty($ybk_anchor_id) && $ybk_anchor_id != -1) {
            $anchor = YbkAnchorService::getByNumber($ybk_anchor_id);
            if (empty($anchor)) {
                throw new ParamInvalidException('主播id错误');
            }
        }
        ...
        try {
            ...
            $order->merchant_type = Order::MERCHANT_TYPE_NORMAL;
            ...
            $order->ybk_anchor_id = $ybk_anchor_id;
            $order->phase_id = 0;
            $order->marketing_id=0;
            $order->original_price = $goods->price * $number;
            if(!empty($canDiscountGoodsList)){
                // 如果有活动商品
                $order->phase_id = $phaseId;
                $order->marketing_id = $marketingId;
                $order->discount_price = $canDiscountGoodsList['discount_price'];
                $limitedNum = min($number,$discountNumber,$canDiscountGoodsList['limited']);
                $order->pay_price = $limitedNum*$canDiscountGoodsList['discount_price'];
                $order->price = $canDiscountGoodsList['discount_price'];
                $order->discount_number = $limitedNum;
                if($number>$limitedNum){
                    // 处理原价数量逻辑
                    $order->pay_price += $goods->price*($number-$limitedNum);
                }
            }
            if($groupBookNumber&&$marketingId){
                // 处理拼团逻辑
                $groupBookData = OrderService::getCanGroupBookListForGroup($phaseId,$marketingId,$groupBookNumber,$goodsId,$groupBookTeamId, $user->id);
                $discountPrice = $groupBookData['discount_price'];
                $order->phase_id = $phaseId;
                $order->marketing_id = $marketingId;
                $order->discount_price = $discountPrice;
                $order->pay_price = $discountPrice*$groupBookNumber;
                $order->price = $discountPrice;
                $order->discount_number = $groupBookNumber;
                $order->marketing_goods_id = $groupBookData['marketing_goods_id'];
                $groupBookOrderId = ($groupBookTeamId) ? $groupBookTeamId : MarketingApi::gerGroupBookNewOrderId($order)['group_book_order_id'];
                if(!$groupBookOrderId) {
                    throw new BaseResponseException('营销端生成拼团订单失败');
                }
                $order->pintuan_order_id = $groupBookOrderId;
            }
            $order->save();
            //团购商品减库存
            $goods->stock -= $number;
...
//            'pay_type' => 'integer|min:1',
        ...
        foreach ($dishesItems as $item) {
            ...
            if (!empty($item->marketing_id) && !empty($item->phase_id)) {
                $marketing_goods_ids[] = $item->dishes_goods_id;
        ...
        if (!empty($marketing_goods_ids)) {
            ...
            $marketing_goods_limit = MarketingApi::getCanDiscountGoodsFromMarketing($dishes->merchant_id,$marketing_goods_ids,$phase_id,$marketing_id,2,$user->id);
            ...
            foreach ($dishesItems as &$item) {
                ...
                if (!empty($item->marketing_id) && !empty($item->phase_id)) {
                    if (!empty($marketing_goods_limit[$item->dishes_goods_id])) {
                        $marketing_goods_info = $marketing_goods_limit[$item->dishes_goods_id];
...
//        $order->pay_type = $payType;
            ...
            $order->bizer_id = $merchant->bizer_id;
            $order->ybk_anchor_id = $ybk_anchor_id;
            $order->marketing_id = $marketing_id;
            $order->phase_id = $phase_id;
            $order->save();
            ...
            foreach ($dishesItems as $item) {
                $dishesGoods = DishesGoods::findOrFail($item->dishes_goods_id);
                $dishesGoods->stock -= $item->number;
                $dishesGoods->save();
                if (!empty($item->marketing_id) && !empty($item->phase_id)) {
                    $orderGoodsList[] = [
                        'goods_id'  =>  $item->dishes_goods_id,
                        'price'     =>  $item->ori_price,
                        'discount_price'=>$item->dishes_goods_sale_price,
                        'buy_number'=>  $item->number,
                        'total_price'=> $item->dishes_goods_sale_price * $item->number,
    ...
    public function csOrderCreate(){
        ...
        $phaseId = request('phase_id', 0);
        $marketingId = request('marketing_id');
        $tempPhaseGoods = request('phase_goods_list','');
        $phaseGoodsList = empty($tempPhaseGoods) ? [] : json_decode($tempPhaseGoods,true);
        $phaseGoodsIds  = empty($phaseGoodsList)? []:array_column($phaseGoodsList,'id');
        $clientGoodNNumberList = empty($phaseGoodsList) ? []:array_combine($phaseGoodsIds,array_column($phaseGoodsList,'number'));
        $groupBookTeamId = request('group_book_team_id', 0);
        ...
        $address = CsUserAddressService::getCsOrderAddress($deliveryType,$addressId);   //获取用户地址
        $merchant = CsMerchantService::getOrderMerchant($merchantId);

        $goodsList = is_string($goodsList) ? json_decode($goodsList, true) : $goodsList;
        $goodList = OrderService::getCsOrderGoodsList($merchant,$goodsList);// 将商品购买量加入到数组对象中去

        $canDiscountGoodsList = OrderService::getCanDiscountGoodsList($phaseId,$marketingId,$phaseGoodsIds,$merchantId,$user->id,OrderService::MARKETING_GOODS_TYPE_SUPERMARKET,$clientGoodNNumberList);           // 可优惠商品
        $goodsPrice = empty($canDiscountGoodsList) ? OrderService::getCsOrderGoodsPrice($goodList) : OrderService::computedDiscountMoneyForPhase($canDiscountGoodsList,$goodList,$clientGoodNNumberList);
        Log::error('记录活动信息：',[$tempPhaseGoods,$phaseId,$marketingId,$phaseGoodsIds,$merchantId,$user->id,$canDiscountGoodsList,$goodsPrice]);
        ...
        try {
            ...
            $order->save();
            //更新商品库存销量
            $hotSellActivity = CsActivityService::getHotSellDetail();
            foreach ($goodList as $item) {
                $good = $item;
                $orderGoodNumber = $good->user_order_num;
                $discountNumber = ($good->discountNumber)??0;
                unset($good->user_order_num,$good->discountNumber);
                $good->sale_num += $orderGoodNumber;
                $good->stock -= $orderGoodNumber;
                $good->save();
                if($marketingId==MarketingApi::MARKETING_GROUP_BOOK) {
                    $groupBookData = OrderService::getCanGroupBookListForSupermarket($item, $orderGoodNumber, $groupBookTeamId, $user->id);
                    // 因为团购只能出现一个商品
                    $csOrderGood = new CsOrderGood();
                    $csOrderGood->oper_id = $merchant->oper_id;
                    $csOrderGood->price = $groupBookData['discount_price'];
                    $csOrderGood->goods_name = $good->goods_name;
                    $csOrderGood->cs_merchant_id = $merchant->id;
                    $csOrderGood->cs_goods_id = $good->id;
                    $csOrderGood->number = $orderGoodNumber;
                    $csOrderGood->order_id = $order->id;
                    $csOrderGood->marketing_id = $marketingId;
                    $csOrderGood->phase_id = $phaseId;
                    $csOrderGood->original_price = $good->price;
                    $csOrderGood->save();
                    $order->marketing_goods_id = $groupBookData['marketing_goods_id'];
                    $groupBookOrderId = ($groupBookTeamId!=0) ? $groupBookTeamId : MarketingApi::gerGroupBookNewOrderId($order)['group_book_order_id'];
                    if(!$groupBookOrderId) {
                        throw new BaseResponseException('营销端生成拼团订单失败');
                    }
                    $order->marketing_id = $marketingId;
                    $order->pintuan_order_id = $groupBookOrderId;
                    $order->pay_price = (float)$groupBookData['discount_price']*$orderGoodNumber + $deliverPrice - $discountPrice;
                    $order->price = (float)$groupBookData['discount_price'];
                    $order->save();
                    break;
                }
                // 判断是否有活动优惠
                $isCanDiscount = ($discountNumber>0);
                if($isCanDiscount){
//                    $goodLimit = $canDiscountGoodsList[$good->id]['limited'];
                    $csOrderGood = new CsOrderGood();
                    $csOrderGood->oper_id = $merchant->oper_id;
                    $csOrderGood->price = $canDiscountGoodsList[$good->id]['discount_price'];
                    $csOrderGood->goods_name = $good->goods_name;
                    $csOrderGood->cs_merchant_id = $merchant->id;
                    $csOrderGood->cs_goods_id = $good->id;
                    $csOrderGood->number = $discountNumber;
                    $csOrderGood->order_id = $order->id;
                    $csOrderGood->marketing_id = $marketingId;
                    $csOrderGood->phase_id = $phaseId;
                    $csOrderGood->original_price = $good->price;
                    $csOrderGood->save();
                    $orderGoodNumber -= $discountNumber;
                }
                if($orderGoodNumber<=0) {
                    continue;
                }
                $csOrderGood = new CsOrderGood();
                $csOrderGood->oper_id = $merchant->oper_id;
                ...
                $csOrderGood->original_price = $good->price;
                $csOrderGood->goods_name = $good->goods_name;
                if ($good->hot_status == CsGood::HOT_STATUS_ON && $hotSellActivity->status == CsActivities::STATUS_ON){
                    $csOrderGood->activity_id = $hotSellActivity->id;
                }
                $csOrderGood->marketing_id = 0;
                $csOrderGood->phase_id = 0;
                $csOrderGood->cs_merchant_id = $merchant->id;
                $csOrderGood->cs_goods_id = $good->id;
                $csOrderGood->number = $orderGoodNumber;
                $csOrderGood->order_id = $order->id;
...
//            throw new BaseResponseException($e->getMessage());
     ...
     * @return JsonResponse
     */
    public function reminderDeliver()
    {
        $this->validate(request(), [
            'id' => 'required|integer|min:1',
        ]);
        $orderId = request('id');
        OrderService::reminderDeliver($orderId);
