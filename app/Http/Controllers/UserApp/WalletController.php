...
class WalletController extends Controller
{
     ...
     */
    public function getFeeSplittingRatioToCsSharePeople()
    {
        $this->validate(request(), [
            'cs_goods_id' => 'required|integer|min:1'
        ]);
        $csGoodsId = request('cs_goods_id');
        $csGoods = CsGoodService::getById($csGoodsId);
        if (empty($csGoods)) throw new BaseResponseException('该商品不存在');
        $merchantId = $csGoods->cs_merchant_id;
        $ratio = FeeSplittingService::getFeeSplittingRatioToCsSharePeopleByMerchantId($merchantId);
        if ($ratio < 0) {
            $ratio = 0;
        }
        $feeSplitting = Utils::floorDecimal(($ratio * $csGoods->price), 2);
        // 返回的是系数 直接乘以金额就好了
        return Result::success([
            'ratio' => $ratio,
