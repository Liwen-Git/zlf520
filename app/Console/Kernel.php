...
use App\Jobs\Cs\HotActStatisticsDaily;
use App\Jobs\Schedule\AutoDownGoodsJob;
use App\Jobs\Schedule\CsGoodsStaDailyJob;
...
use App\Jobs\Schedule\YbkStatistics;
...
//        $schedule->job(OrderAutoConfirmed::class)->hourly();
        ...
        /**超市商户月售统计*/
        $schedule->job(CsMerchantOrder::class)->daily();
        /**超市商品最近30天销售数量统计*/
        $schedule->job(CsGoodsStaDailyJob::class)->daily();
        /**超市年货节 每日统计*/
        $schedule->job(new HotActStatisticsDaily(Carbon::now()->subDay()->endOfDay()))->daily();
//        $schedule->job(HotActStatisticsDaily::dispatch(Carbon::now()->subDay()->endOfDay()))->daily();
        /** 云博客数据统计  */
        $schedule->job(YbkStatistics::class)->everyThirtyMinutes();
    }
