...
class CsMerchantOrder implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        $sql = "SELECT a.id,b.c from cs_merchants a left JOIN
(
SELECT merchant_id,count(*) c from orders where merchant_type=2 and created_at>'{$start_date}' and `status` in ({$status})
GROUP BY merchant_id
        ...
        if ($rs) {
            foreach ($rs as $v) {
                $row = CsStatisticsMerchantOrderService::createMerchantData($v->id);
                $row->order_number_today = 0;
                $row->order_number_30d = $v->c??0;
                $row->save();
            }
        }
