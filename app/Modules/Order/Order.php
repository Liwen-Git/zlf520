 ...
 * @property Carbon reminder_time
 * @property string  ybk_anchor_id
 * @property string  ybk_anchor_number
 * @property integer auto_take_deliver
 * @property integer share_user_id
 * @property integer marketing_id
 * @property integer phase_id
 * @property integer discount_number
 * @property integer pintuan_order_id
 * @property integer marketing_goods_id
 */
...
class Order extends BaseModel
{
    ...
    const TYPE_SCAN_QRCODE_PAY_WITH_PRICE = 5; // 扫码支付（带价格）
    ...
    const STATUS_PAID_UNSETTLED = 11; // 已支付未成交    用于拼团单记录已支付但未拼团成功
    const STATUS_REFUND_FAIL = 12;  // 退款失败
    ...
    {
        return ['', '未支付', '已取消', '已关闭[超时自动关闭]', '已支付', '退款中[保留状态]', '已退款', '已完成', '待发货', '待自提', '已发货', '已付款', '退款失败'][$status];
    }

    public static function getMarketingText($marketingId)
    {
        return ['无', '限时抢购', '拼团'][$marketingId];
    }
    ...
    public static function getAllGoodsName($type, $goodsItems)
    {
        $goodsName = '';
        if($type == self::TYPE_DISHES && count($goodsItems) > 0){
            foreach ($goodsItems as $index => $goodsItem) {
                $i = $index + 1;
                $goodsName .= "$i.$goodsItem->dishes_goods_name * $goodsItem->number \n";
            }
        }elseif ($type == self::TYPE_SUPERMARKET && count($goodsItems) > 0) {
            foreach ($goodsItems as $index => $goodsItem) {
                $i = $index + 1;
                $goodsName .= "$i.$goodsItem->goods_name * $goodsItem->number \n";
            }
        }else {
            $goodsName = '无';
        }
    ...
    public function ybkAnchor()
    {
        return $this->belongsTo(YbkAnchor::class, 'ybk_anchor_id', 'ybk_pk');
    }
    ...
    public function dishesItems()
    {
        return $this->hasMany(DishesItem::class, 'dishes_id', 'dishes_id');
    }
}
