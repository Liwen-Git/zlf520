...
class SettlementService extends BaseService
{
     ...
     * 商户端app 获取商户的结算列表
     * @param $params
     * @return SettlementPlatform[]|\Illuminate\Database\Eloquent\Collection|\Illuminate\Database\Query\Builder[]|\Illuminate\Support\Collection
     */
    public static function merchantAppGetList($params)
    {
        $merchantId = array_get($params, 'merchantId');
        $pageSize = array_get($params, 'pageSize', 10);
        $page = array_get($params, 'page', 1);

        $settlement = Settlement::where('merchant_id', $merchantId)
            ->select('id', 'merchant_id', 'start_date', 'end_date', 'amount', 'real_amount', 'status', 'created_at', 'updated_at')
            ->selectRaw("1 as pay_target_type");
        $data = SettlementPlatform::where('merchant_id', $merchantId)
            ->where('merchant_type', SettlementPlatform::MERCHANT_TYPE_NORMAL)
            ->select('id', 'merchant_id', 'start_date', 'end_date', 'amount', 'real_amount', 'status', 'created_at', 'updated_at')
            ->selectRaw("2 as pay_target_type")
            ->union($settlement)
