...
class SettlementPlatformService extends BaseService
{
    ...
    public static function getListForSaas(array $params = [],bool $return_query = false)
    {
        ...
        if($params['merchantType']==SettlementPlatform::MERCHANT_TYPE_CS){
            $query->whereHas('cs_merchant',function($q) use ($params) {
        ...
        if (!empty($params['oper_name'])) {
            $query->whereHas('oper',function($q) use ($params) {
                $q->where('name', 'like', "%{$params['oper_name']}%");
            });
        }
        //展示所有结算周期的结算单
        ...
        if (!empty($params['settlement_no'])) {
            $query->where('settlement_no',$params['settlement_no']);
        }

        if ($params['manual_gen']) {
            $query->where('gen_type',2);
        }
    ...
    public static function getKuaiQianBatchesStatus($id)
    {
        $status = 0;
        $platformInfo = SettlementPlatform::find($id);
        if($platformInfo){
           $batchInfo = SettlementPlatformKuaiQianBatch::where('batch_no',$platformInfo->pay_batch_no)->first();
           if($batchInfo){
    ...
    public static function manualStatus($id)
    {
        $stauts = self::getKuaiQianBatchesStatus($id);
        if($stauts == SettlementPlatformKuaiQianBatch::STATUS_FINISHED){
            SettlementPlatform::where('id',$id)
                ->update([
                    'remark'=>'人工重新打款',
                    'status'=>3,
                    ]);
     ...
     * @throws \Exception
     */
    public static function genBatchByDates($dates = [])
    {
        if (empty($dates)) {
            return false;
        }
        ...
        foreach ($dates as $date) {
            ...
            $settlement_platform = SettlementPlatform::where('pay_batch_no','')
                ...
                ->where('merchant_type',SettlementPlatform::MERCHANT_TYPE_CS)
    ...
    public static function genBatchAgain($settlement_id)
    {
        ...
        if ($settlement_info->merchant_type == SettlementPlatform::MERCHANT_TYPE_NORMAL) {
            $merchant = Merchant::findOrFail($settlement_info->merchant_id);
        } elseif ($settlement_info->merchant_type == SettlementPlatform::MERCHANT_TYPE_CS) {
            $merchant = CsMerchant::findOrFail($settlement_info->merchant_id);
        } else {
            throw new BaseResponseException('结算单类型错误');
        }
