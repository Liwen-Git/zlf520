...
//            'aes_key'  => '开放平台第三方平台 AES Key'
     ...
     */
    public static function genMiniprogramAppCode($operId, $sceneId, $page='pages/index/index', $width=375, $getImage=false,$name ='')
    {
        if($operId){
        ...
        $image = self::addSceneIdToAppCode($image, $sceneId);
        $image = self::addNameToAppCode($image,$name);

        if($getImage){
            return $image;
        }

        $filename = "/qrcode/scene_app_code/_{$sceneId}_{$width}.jpg";
    ...
    public static function getMiniprogramAppCodeUrl($sceneId)
    {
        ...
        if(!empty($scene->qrcode_url)){
            ...
            $name = '';
            if($scene->merchant_id){
                $name = MerchantService::getSignboardNameById($scene->merchant_id);
            }
            $url = self::genMiniprogramAppCode($scene->oper_id, $scene->id, $scene->page,'',false, $name);
    ...
    /**
     * 给小程序码增加场景ID
     * @param \Intervention\Image\Image $appCode
     * @param int|string $sceneId
     * @return \Intervention\Image\Image
     */
    public static function addSceneIdToAppCode($appCode, $sceneId)
    {
        // 设置基础比率 (字体大小与放大比例)
     ...
     * @return \Intervention\Image\Image
     */
    public static function addNameToAppCode($appCode, $name)
    {
        // 设置基础比率 (字体大小与放大比例)
