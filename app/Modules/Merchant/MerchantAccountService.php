...
class MerchantAccountService extends BaseService
{
    ...
    public static function login($username,$password){
        ...
        if($user->type == MerchantAccount::TYPE_NORMAL){
            ...
            $user->operName = OperService::getNameById($merchant->oper_id);
            $user->token = md5($user->merchantName . time());
            $user->marketing_url = config('common.marketing_domain') . '/front/merchant?token=' . $user->token;
        }else {
            // 大千超市信息获取
            ...
            $user->operName = OperService::getNameById($csMerchant->oper_id);
            $user->token = md5($user->merchantName . time());
            $user->marketing_url = config('common.marketing_domain') . '/front/cs?token=' . $user->token;
        }

        DataCacheService::setLoginCache('merchant',$user);
        // 将用户信息记录到session中
        session([
...
//                [ 'id' => 8, 'name' => '电子合同管理', 'level' => 1, 'url' => '/cs/electronic/contract',],
                ...
                [ 'id' => 9, 'name' => '平台活动', 'level' => 1, 'url' => 'activity', 'sub' =>
                    [
                        [ 'id' => 901, 'name' => '年货节活动详情', 'level' => 2, 'url' => '/cs/activity_hot/activityDetail', 'pid' => 9,],
                        [ 'id' => 902, 'name' => '年货节活动统计', 'level' => 2, 'url' => '/cs/activity_hot/statistics', 'pid' => 9,],
                    ]
                ],

                [ 'id' => 10, 'name' => '系统设置', 'level' => 1, 'url' => 'setting', 'sub' =>
                    [
                        [ 'id' => 1001, 'name' => '配送设置', 'level' => 2, 'url' => '/cs/setting/delivery', 'pid' => 10 ],
                    ]
                ],
            ];
            //年货节活动不启用时，隐藏菜单
            /*if(!\App\Modules\Cs\CsActivityService::checkEnableCsActivity()){
                foreach ($menus as $key => $item){
                    if($item['id'] == 9){
                        foreach ($item['sub'] as $subKey => $subItem){
                            if($subItem['id'] == 901){
                                unset($menus[$key]['sub'][$subKey]);
                            }
                        }
                        $menus[$key]['sub'] = array_values($menus[$key]['sub']);
    ...
    public static function appLogin($username, $password)
    {
        ...
        if($user->status != 1){
            throw new NoPermissionException('帐号已被冻结，请联系相关人员');
        }
        ...
        if ($merchant->audit_status == Merchant::AUDIT_STATUS_AUDITING) {
            throw new NoPermissionException('账号正在审核中......');
        }
        $user->merchantName = $merchant->name;
        $user->mobile = $merchant->contacter_phone;

        // 如果用户存在旧token，则清除，限制用户只能登陆1终端
        if( Cache::has('app_merchant_user_id_to_token_' . $user->id)){
            $oldToken = Cache::get('app_merchant_user_id_to_token_' . $user->id);
            Cache::forget('app_merchant_user_id_to_token_' . $user->id);
            Cache::forget('token_to_app_merchant_' . $oldToken);
        }
        // 生成token并返回
        $token = str_random(64);
        Cache::put('token_to_app_merchant_' . $token, $user, 60 * 24 * 30);
        Cache::put('app_merchant_user_id_to_token_' . $user->id, $token, 60 * 24 * 30);
