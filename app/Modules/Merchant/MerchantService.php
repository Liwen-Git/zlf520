...
//        })
    ...
    public static function getList(array $data, bool $getWithQuery = false)
    {
        ...
        $merchant_cat_id = array_get($data, 'merchant_cat_id');
        ...
        if (!empty($merchant_cat_id)) {
            if (is_array($merchant_cat_id) || $merchant_cat_id instanceof Collection) {
                $query->whereIn('merchant_category_id', $merchant_cat_id);
            } else {
                $query->where('merchant_category_id', $merchant_cat_id);
            }
        }
    ...
    public static function detail($id)
    {
        ...
        $merchantFollow = MerchantFollow::where('user_id', $userId)
            ->where('merchant_id', $id)
            ->where('merchant_type', MerchantFollow::MERCHANT_TYPE_NORMAL)
            ->first();
        $merchant->user_follow_status = empty($merchantFollow) ? MerchantFollow::USER_NOT_FOLLOW : $merchantFollow->status;
    ...
    public static function getListAndDistance(array $params = [])
    {
        ...
        $list->each(function ($item){
            ...
            $item->merchantCategoryPid = $category->pid;
            // 最低消费
