...
class MerchantCategoryService extends BaseService
{
    ...
    public static function add($name, $icon = '', $status = 1, $pid = 0)
    {
        ...
        $maxSort = MerchantCategory::max('sort') + 1;
        $category->name = $name;
        $category->icon = $icon;
        $category->status = $status;
        $category->pid = $pid;
        $category->sort = $maxSort;
        $category->save();
        self::clearCache();
    ...
    {
        $subArray = MerchantCategory::where([['pid', $categoryId]])->orderBy('sort')->get();
        if (count($subArray) > 0){
            return $subArray;
        }else{
            return false;
    ...
    public static function up($id){
        ...
        $isChanged = false;
        if($category){
            $categoryList = self::getSubCategories($category->pid);
            foreach ($categoryList as $index => $item){
                if($category->id == $item->id){
                    if($index > 0){
                        $prevCategory = $categoryList[$index - 1];
                        MerchantCategory::where('id', $category->id)->update(['sort' => $prevCategory->sort]);
                        MerchantCategory::where('id', $prevCategory->id)->update(['sort' => $category->sort]);
                        $isChanged = true;
                        self::clearCache();
                    }
                    break;
    ...
    public static function down($id){
        ...
        if($category){
            ...
            foreach ($categoryList as $index => $item){
                if($category->id == $item->id){
                    if($index < count($categoryList) -1){
                        $nextCategory = $categoryList[$index + 1];
                        MerchantCategory::where('id', $category->id)->update(['sort' => $nextCategory->sort]);
                        MerchantCategory::where('id', $nextCategory->id)->update(['sort' => $category->sort]);
                        $isChanged = true;
                        self::clearCache();
                    }
                    break;
