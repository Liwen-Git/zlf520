...
class CsMerchantService extends BaseService {
    ...
    const FIRST_SHOW_MERCHANT_ID = 1000000334;          // 首先展示merchant_id
    ...
    public static function getListHot(array $params, bool $getWithQuery = false)
    {

        $query = CsMerchant::select('*')
            ->whereNotNull('hot_add_time')
            ->when(!empty($params['cs_merchant_id']),function (Builder $query) use ($params){
                $query->where('id','=',$params['cs_merchant_id']);
            })
            ...
            ->when(!empty($params['hot_status']),function (Builder $query) use ($params){
                $query->whereIn('hot_status',$params['hot_status']);
            })
            ->when(!empty($params['merchant_name']),function (Builder $query) use ($params){
                $query->where('name','like',"%{$params['merchant_name']}%");
            })
    ...
    public static function getHotSellListAndDistance(array $params=[])
    {
        ...
        $query = CsMerchant::where('status', CsMerchant::STATUS_ON)
            ->where('oper_id', '>', 0)
            ->where('hot_status',CsMerchant::HOT_STATUS_ON)
            ...
            ->when(!empty($params['city_id']),function (Builder $query) use ($params){
                // 城市区间ID优先排序
                /*
                $query->select('*')
                    ->selectRaw('abs(city_id - ?) as city_id_count',[$params['city_id']])
                    ->orderBy('city_id_count','asc');
                */
        ...
        // 补充商家其他信息
        $list = collect($list);
        $list->each(function ($item) use($lng,$lat,$user_key){
            $item->desc_pic_list = $item->desc_pic_list ? explode(',', $item->desc_pic_list) : [];
            ...
            if(CsActivityService::isOpenHotSell()){
                $hotGoods = CsGood::where('cs_merchant_id',$item->id)
                    ...
                    ->get();
                $item->hot_goods = $hotGoods;
            }

            //配送信息:
            self::getMerchantSetting($item);
            $item->distance =($lng && $lat) ? Lbs::getDistanceOfCsMerchant($item->id, $user_key, floatval($lng), floatval($lat)):0;
            if($item->distance!=0){
                $item->distance = Utils::getFormativeDistance($item->distance);
            }
        });
    ...
    public static function getOtherHotSellList(array $params=[])
    {
        $city_id = array_get($params, 'city_id',0);
        $keyword = array_get($params, 'keyword',null);
        $pageSize = array_get($params, 'pageSize',15);
        ...
        // 补充商家其他信息
        $list = collect($list);
        $list->each(function ($item) use($user_key){
            $item->desc_pic_list = $item->desc_pic_list ? explode(',', $item->desc_pic_list) : [];
            if($item->business_time) $item->business_time = json_decode($item->business_time, 1);
            // 兼容v1.0.0版客服电话字段
            $item->contacter_phone = $item->service_phone;
    ...
    /**
     * 获取商家列表可以计算距离(非1.5.2版本)
     * @param array $params
     * @param bool $isOpenHotSelf   是否开启年货节
     * @return array
     */
    public static function getListAndDistanceOld(array $params = [])
    {
    ...
    public static function getListAndDistance(array $params = [])
    {
        ...
        $query = CsMerchant::where('status', CsMerchant::STATUS_ON)
            ->where('oper_id', '>', 0)
            ->whereIn('audit_status', [CsMerchant::AUDIT_STATUS_SUCCESS])
            ->where('id','!=',self::FIRST_SHOW_MERCHANT_ID)         // 不查询第一个展示商家
            ->when($city_id, function(Builder $query) use ($city_id){
                // 特殊城市，如澳门。属于省份，要显示下属所有城市的商户
                $areaInfo = Area::where('area_id', $city_id)->where('path', 1)->first();
                if (empty($areaInfo)) {
        ...
        // 添加第一个显示商户
        $firstShowMerchant = CsMerchantService::getById(self::FIRST_SHOW_MERCHANT_ID);
        $list = collect($list);
        if($firstShowMerchant){
            $total++;
            if($params['page']==1){
                $list=$list->prepend($firstShowMerchant);
            }
        }
        $list->each(function ($item){
    ...
    public static function getMerchantSetting(&$supermarket){
        //配送信息:
...
//            $supermarket->delivery_free_order_amount = ($MerchantInfo->delivery_free_start!=0) ? $MerchantInfo->delivery_free_order_amount:1000000000;
            $supermarket->delivery_free_order_amount =$MerchantInfo->delivery_free_order_amount;
        }
    ...
    public static function otherCityList($params) {
        $city_id = array_get($params, 'city_id');
        $keyword = array_get($params, 'keyword',null);
        $pageSize = array_get($params, 'pageSize',15);
        ...
        $query = CsMerchant::where('status', CsMerchant::STATUS_ON)
            ->where('oper_id', '>', 0)
            ->whereIn('audit_status', [CsMerchant::AUDIT_STATUS_SUCCESS])
            ->where('id','!=',self::FIRST_SHOW_MERCHANT_ID)     // 不展示第一个商家
            ->when($city_id, function(Builder $query) use ($city_id){
                //城市之外的商户
                $query->where('city_id','!=', $city_id);
     ...
     * @return CsMerchant
     */
    public static function getOrderMerchant($merchantId)
    {
     ...
     * @return CsMerchant[]|\Illuminate\Contracts\Pagination\LengthAwarePaginator|Builder[]|Collection|\Illuminate\Database\Query\Builder[]|\Illuminate\Support\Collection
     */
    public static function getListByName($name, $returnAll = false)
    {
        $query = CsMerchant::where('cs_merchants.audit_status', CsMerchant::AUDIT_STATUS_SUCCESS)
            ->where('cs_merchants.status', CsMerchant::STATUS_ON)
            ->where('cs_merchants.signboard_name', 'like', "%$name%")
            ->leftJoin('cs_statistics_merchant_orders', 'cs_merchants.id', '=', 'cs_statistics_merchant_orders.cs_merchant_id')
            ->select('cs_merchants.*')
            ->selectRaw('cs_statistics_merchant_orders.order_number_30d + cs_statistics_merchant_orders.order_number_today as order_count_30d')
            ->orderBy('order_count_30d', 'desc');

        if ($returnAll) {
            return $query->get()->each(function ($item) {
                $item->load(['csGoods' => function ($query) {
        ...
        $data->each(function ($item){
            ...
            //超市活动商品信息
            if ($item->csGoods->isNotEmpty()) {
                $item->csGoods->each(function ($val) {
                    $goods = MarketingApi::getInActivityMarketingGoods($val->id, MarketingApi::MARKETING_GOODS_TYPE_CS_GOODS);
                    if (!empty($goods)) {
                        $val->is_in_markting = 1;
                        $val->marketing_id = $goods['marketing_id'];
                        $val->discount_price = $goods['discount_price'];
                    } else {
                        $val->is_in_markting = 0;
                        $val->marketing_id = 0;
