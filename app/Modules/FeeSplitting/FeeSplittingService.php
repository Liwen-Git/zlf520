...
class FeeSplittingService extends BaseService
{
     ...
     * @throws \Exception
     */
    private static function feeSplittingToSelfAndSharePeople(Order $order, float $profitAmount)
    {
        DB::beginTransaction();
        try {
            if ($order->share_user_id) {
                // 用户自己5%
                $user = UserService::getUserById($order->user_id);
                $userRatio = 5;
                $userFeeSplittingRecord = self::createFeeSplittingRecord($order, $user, FeeSplittingRecord::TYPE_TO_SELF, $profitAmount, $userRatio);
                $userWallet = WalletService::getWalletInfo($user);
                WalletService::addFreezeBalanceByFeeSplitting($userFeeSplittingRecord, $userWallet);

                // 分享人15%
                $shareUser = UserService::getUserById($order->share_user_id);
                $shareRatio = UserCreditSettingService::getFeeSplittingRatioToSharePeopleOfCs();
                $shareFeeSplittingRecord = self::createFeeSplittingRecord($order, $shareUser, FeeSplittingRecord::TYPE_TO_CS_SHARE, $profitAmount, $shareRatio);
                $shareWallet = WalletService::getWalletInfo($shareUser);
                WalletService::addFreezeBalanceByFeeSplitting($shareFeeSplittingRecord, $shareWallet);
            } else {
                // 用户自己20%
                $user = UserService::getUserById($order->user_id);
                $userRatio = 20;
    ...
    private static function createFeeSplittingRecord(Order $order, $originInfo, $type, float $profitAmount, float $feeRatio = 0)
    {
        ...
        if ($type == FeeSplittingRecord::TYPE_TO_SELF) {
            ...
            if ($order->merchant_type != Order::MERCHANT_TYPE_SUPERMARKET) {
                $feeRatio = UserCreditSettingService::getFeeSplittingRatioToSelfSetting(); // (非超市订单)自反的分润比例
            } else {
                $feeRatio = UserCreditSettingService::getFeeSplittingRatioToSelfOfCs(); // 超市订单的自反比例
            }
            ...
            if ($order->merchant_type != Order::MERCHANT_TYPE_SUPERMARKET) {
                if ($originType == FeeSplittingRecord::ORIGIN_TYPE_USER) {
                    ...
                    $feeRatio = UserCreditSettingService::getFeeSplittingRatioToParentOfCsInNormalOrder();
                } else {
                    throw new BaseResponseException();
                }
            } else {
                $feeRatio = UserCreditSettingService::getFeeSplittingRatioToParentOfCsInCsOrder(); // 超市订单 返给上级比例
            }
        } elseif ($type == FeeSplittingRecord::TYPE_TO_OPER) {
            // 3 运营中心分润比例
        } elseif ($type == FeeSplittingRecord::TYPE_TO_BIZER) {
            // 4 业务员分润比例
        } elseif ($type == FeeSplittingRecord::TYPE_TO_CS_SHARE) {
            // 5 超市分享人分润
        }
        else {
    ...
    public static function getUserFeeSplittingRatioToSelfByMerchantId($merchantId)
    {
        ...
        if ($merchantId >= 1000000000) {
            $operId = CsMerchant::where('id', $merchantId)->value('oper_id');
        } else {
        ...
        if ($merchantId >= 1000000000) {
            return self::userFeeSplittingRatioOfCs($merchant->settlement_rate);
        } else {
            return self::UserFeeSplittingRatio($merchant->settlement_rate);
        }
    }

    public static function UserFeeSplittingRatio($settlementRate)
    {
     ...
     * @return float|int
     */
    public static function getFeeSplittingRatioToCsSharePeopleByMerchantId($merchantId)
    {
        $csMerchant = CsMerchantService::getById($merchantId);
        if (empty($csMerchant)) {
            throw new BaseResponseException('该超市商户不存在');
        }
        $oper = OperService::getById($csMerchant->oper_id);
        if ($oper->pay_to_platform == Oper::PAY_TO_OPER) {
            throw new BaseResponseException('该超市商户的运营中心是支付到平台');
        }
        $feeRatio = UserCreditSettingService::getFeeSplittingRatioToSharePeopleOfCs(); // 分享人的分润比例
        $ratio = self::calculationRatio($feeRatio, $csMerchant->settlement_rate);
     ...
     * @return float|int
     */
    public static function userFeeSplittingRatioOfCs($settlementRate)
    {
        // 超市用户自反 的分润比例
        $feeRatio = UserCreditSettingService::getFeeSplittingRatioToSelfOfCs();
        $ratio = self::calculationRatio($feeRatio, $settlementRate);
