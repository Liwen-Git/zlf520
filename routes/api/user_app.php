...
if (!empty($version)) {
    ...
    if (count($tmp) == 3) {
        if ($tmp[0]>1) {
            $namespace .= '\\V' . (str_replace('.','_',$version));
        } elseif ($tmp[0] == 1 && $tmp[1]>5) {

            $namespace .= '\\V' . (str_replace('.','_',$version));
        }elseif ($tmp[0] == 1 && $tmp[1]==5 && $tmp[2] > 0) {
...
Route::prefix('app/user')
    ...
    ->middleware('user_app')->group(function () {
        ...
        Route::get('dishes/goods/detail', 'DishesController@goodsDetail');
        ...
        Route::post('order/reminderDeliver','OrderController@reminderDeliver')->middleware(UserLoginFilter::class);

        Route::post('order/newGroupBuy', 'OrderNewController@groupBuy')->middleware(UserLoginFilter::class);
        Route::post('order/newMoneyBuy', 'OrderNewController@moneyBuy')->middleware(UserLoginFilter::class);
        Route::post('order/newSupermarketBuy', 'OrderNewController@supermarketBuy')->middleware(UserLoginFilter::class);
        Route::post('order/newSingleBuy','OrderNewController@singleBuy')->middleware(UserLoginFilter::class);
        Route::post('order/newPay','OrderNewController@pay')->middleware(UserLoginFilter::class);
        ...
        Route::get('wallet/getFeeSplittingRatioToCsSharePeople', 'WalletController@getFeeSplittingRatioToCsSharePeople')->middleware(UserLoginFilter::class);
        ...
        Route::get('cs/merchant/otherCityList','CsMerchantController@otherCityList');
        ...
        Route::get('cs/merchant/byName','CsMerchantController@getCsMerchantListByName');

        Route::get('cs/merchant/goods','CsGoodsController@getAllGoods');
        Route::get('cs/goods/detail','CsGoodsController@detail');
        Route::get('cs/goods/byName', 'CsGoodsController@getCsGoodsListByName');
        ...
        Route::get('cs/merchant/hotList','CsMerchantController@getHotList');
        Route::get('cs/merchant/otherHotList','CsMerchantController@getOtherHotList');
        Route::get('cs/activity/hotSell','CsActivityController@getHotSell');
        Route::get('cs/activity/getHotGoods','CsActivityController@getHotGoods');
        ...
        Route::get('ybk/searchAnchor','YbkController@searchAnchor');

        Route::get('ybk/getList','YbkController@getList');
        Route::get('ybk/orderDetail','YbkController@orderDetail');

        Route::get('poster','QRCodePosterController@getPoster');
        Route::get('poster/test','QRCodePosterController@test');

        Route::get('marketing/getGoodsListStart','MarketingController@getGoodsListStart');
        Route::get('marketing/getGoodsListSoon','MarketingController@getGoodsListSoon');
        Route::get('marketing/getMerchantGoodsListStart','MarketingController@getMerchantGoodsListStart');
        Route::get('marketing/userView','MarketingController@userView');
        Route::get('marketing/getMarketingInfo','MarketingController@getMarketingInfo');

        Route::get('dqad/getByPosition','AdController@getByPosition');

        Route::get('groupBook/getGoodsList','GroupBookController@getGoodsList');
        Route::get('groupBook/orders','GroupBookController@getOrdersList')->middleware(UserLoginFilter::class);
