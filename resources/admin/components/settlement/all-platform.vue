<template>
    ...
    <page title="商户货款结算管理" v-loading="isLoading">
        ...
        <el-col>
            <el-form v-model="query" inline>
                ...
                <el-form-item prop="oper_name" label="运营中心名称" >
                    <el-input v-model="query.oper_name" size="small"  placeholder="运营中心名称"  class="w-150" clearable></el-input>
                </el-form-item>
                ...
                <el-form-item prop="settlement_no" label="结算单号" >
                    <el-input v-model="query.settlement_no" size="small"  placeholder="结算单号"  class="w-200" clearable></el-input>
                </el-form-item>
                <el-form-item prop="manual_gen" label="" >
                    <el-checkbox v-model="query.manual_gen">强制生成结算单</el-checkbox>
                </el-form-item>
                <el-form-item>
        ...
        <el-dialog :visible.sync="isShowSettlementDetail"  width="70%">
            <settlement-detail :scope="settlement"/>
        </el-dialog>

        <el-table :data="list" v-loading="tableLoading" stripe>
            <el-table-column prop="settlement_no" label="结算单号"  width="170px" />
            <el-table-column prop="merchant.id" label="商户ID"  width="70px" />
            <el-table-column prop="merchant.name" label="商户名称"  width="160px" />
            <el-table-column prop="merchant.merchant_type" label="商户类型">
            ...
            <el-table-column prop="reason" label="强制生成" >
                <template slot-scope="scope">
                    <span v-if="scope.row.gen_type === 2" >是</span>
                    <span v-else>否</span>
                </template>
            </el-table-column>
            ...
            <el-table-column label="操作" width="180px">
                <template slot-scope="scope">
                    ...
                    <el-button type="text" v-if="hasRule('/api/admin/settlement/modifyStatus') && parseInt(scope.row.status) === 1 && parseInt(scope.row.settlement_cycle_type) !== 3" @click="modifyPlatformStatus(scope)">确认打款</el-button>
                    <!--<el-button type="text" v-if="hasRule('/api/admin/settlement/modifyStatus') && parseInt(scope.row.status) === 4 && parseInt(scope.row.settlement_cycle_type) === 3" @click="genBatchAgain(scope)">重新打款</el-button>-->
                    <el-button type="text" v-if="hasRule('/api/admin/settlement/manualStatus') && parseInt(scope.row.status) === 4 && parseInt(scope.row.settlement_cycle_type) === 3" @click="manualStatus(scope)">人工重新确认打款</el-button>
                </template>
            </el-table-column>
        </el-table>
        <el-pagination
        ...
        methods: {
            ...
            downloadExcel() {
                ...
                this.$confirm(message).then(() => {
                    ...
                    location.href = '/api/admin/settlement/download?' + array.join('&');
                })
            },
            ...
            genBatchAgain(scope){
                ...
                this.$confirm('确定发起重新打款指令吗').then(() => {
                    api.get('/settlement/reBatchAgain', {id: scope.row.id});
                    this.getList();
                })
            },
            manualStatus(scope){
                api.get('/settlement/getStatus', {id: scope.row.id}).then(data => {
                    if(data.status == 2){
                        this.$confirm('确认此单已完成打款了吗，请再次确认').then(() => {
                            api.get('/settlement/manualStatus', {id: scope.row.id}).then(data => {
                                this.getList();
                            });
                        })
                    }else{
                        this.$message({
                            message: '该批次正在处理中，请稍后再试',
                            type: 'warning'
                        });
                    }
                    this.tableLoading = false;
