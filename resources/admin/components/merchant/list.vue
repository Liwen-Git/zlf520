    ...
    <page :title=" isAudit ? '审核商户': '商户列表'"     v-loading="isLoading" >
        <el-col>
            <el-form v-model="query" inline>
                ...
                <!--</el-form-item>-->
                <el-form-item label="行业" prop="cs_platform_cat_id_level1">
                    <template>
                        <el-select size="small" v-model="query.merchant_cat_id_1_level1" clearable  placeholder="请选择" @change="getLevel2()">
                            <el-option
                                    v-for="item in merchant_cat_id_1_level1"
                                    :key="item.value"
                                    :label="item.label"
                                    :value="item.value">
                            </el-option>
                        </el-select>
                        <el-select size="small" v-model="query.merchant_cat_id_1_level2" clearable  placeholder="请选择">
                            <el-option
                                    v-for="item in merchant_cat_id_1_level2"
                                    :key="item.value"
                                    :label="item.label"
                                    :value="item.value">
                            </el-option>
                    ...
                    <template>
                        ...
                        <el-dropdown trigger="click" @command="(command) => {audit(scope, command)}">
                            ...
                            <el-dropdown-menu slot="dropdown">
                                <el-dropdown-item v-if="parseInt(scope.row.audit_status)===0 || parseInt(scope.row.audit_status)===2" command="1">审核通过</el-dropdown-item>
                                <el-dropdown-item v-if="parseInt(scope.row.audit_status)===0 || parseInt(scope.row.audit_status)===1" command="2">审核不通过</el-dropdown-item>
...
<script>
    ...
    export default {
        ...
        data(){
            return {
                ...
                query: {
                    ...
                    memberNameOrMobile: '',
                    bizerNameOrMobile: '',
                    merchant_cat_id_1_level1: '',
                    merchant_cat_id_1_level2: '',
                },
                list: [],
                auditRecord:[],
                total: 0,
        ...
        methods: {
            ...
            getLevel1() {
                api.get('/merchant/sub_cat', {parent_id:0}).then(data => {

                    this.merchant_cat_id_1_level1 = data;
                })
            },
            getLevel2() {
                if (this.query.merchant_cat_id_1_level1 == 0) {
                    return true;

                }
                api.get('/merchant/sub_cat', {parent_id:this.query.merchant_cat_id_1_level1}).then(data => {
                    this.query.merchant_cat_id_1_level2 = ''
                    this.merchant_cat_id_1_level2 = data;
                })
            },
        },
        created(){
