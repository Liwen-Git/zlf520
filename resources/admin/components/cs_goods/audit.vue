        ...
        <el-row>
            <el-col :span="22">
                <el-form label-width="120px">
                    ...
                    <el-form-item>
                        <el-button type="success" @click="audit(1)" v-if="goods.audit_status == 3">审核通过</el-button>
                        <el-button type="warning" @click="audit(2)" v-if="goods.audit_status == 2">审核不通过</el-button>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            audit(type){
                api.post('/cs/goods/audit', {id: this.id, type: type,audit_suggestion:this.goods.audit_suggestion}).then(data => {
                    this.$message.success(['', '审核通过', '审核不通过', '打回商户池'][type] + ' 成功');
                    this.back()
                })
            },
            getDetail(){
                api.get('cs/goods/detail', {id: this.id,}).then(data => {
                    this.goods = data;
                });
            },
            back(){
