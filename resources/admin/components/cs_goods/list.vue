    ...
    <page title="商品管理" v-loading="isLoading">
        <el-col>
            <el-form v-model="query" inline>
                ...
                <el-form-item>
                    <el-button type="danger" @click="dialogVisible = true" >批量审核不通过</el-button>
                </el-form-item>
                <el-form-item>
        ...
        <el-table :data="list" stripe v-loading="dataLoading" @selection-change="handleSelectionChange" ref="multipleTable">
            <el-table-column
                type="selection"
            ...
            <el-table-column prop="audit_status" label="审核状态">
                <template slot-scope="scope">
                    <span v-if="parseInt(scope.row.audit_status) === 1" class="c-warning">待审核</span>
                    <div v-else-if="parseInt(scope.row.audit_status) === 2"  slot="reference" class="c-green"><p>审核通过</p></div>
                    <div  v-else-if="parseInt(scope.row.audit_status) === 3" slot="reference" class="c-danger"><p v-if="parseInt(scope.row.saas_audit_status) === 2">平台抽检不通过</p><p v-else>审核不通过</p><span class="message">{{scope.row.audit_suggestion}}</span></div>
                    <span v-else>未知 ({{scope.row.status}})</span>
                </template>
            </el-table-column>
        ...
        <el-dialog
            ...
            <span>共{{this.selectedCount}}个商品<br/></span>
            <span>确认审核不通过吗？</span>
            <span slot="footer" class="dialog-footer">
            <el-button @click="dialogVisible = false">取 消</el-button>
            <el-button type="primary" @click="auditAllNotPassed" >确 定</el-button>
          </span>
        </el-dialog>
    ...
    export default {
        ...
        methods: {
            ...
            getLevel2() {
                ...
                api.get('/cs/sub_cat', {parent_id:this.query.cs_platform_cat_id_level1}).then(data => {
            ...
            getList(){
                ...
                api.get('/cs/goods', this.query).then(data => {
                    this.list = data.list;
            ...
            downloadExcel() {
                ...
                this.$confirm(message).then(() => {
                    ...
                    location.href = `/api/admin/cs/goods/download?${uri}`;
                })
            },
            auditAllNotPassed(){
                ...
                for(var key in this.multipleSelection){
                    //console.log(this.multipleSelection[key]);
                    selectedIds.push(this.multipleSelection[key].id);
                }
                let data = {
                    'ids':selectedIds
                };
                ...
                api.post('/cs/goods/auditAllNotPassed', data).then((data) => {
                        this.$message({
                            message: '下架成功',
                            type: 'success'
                        });
