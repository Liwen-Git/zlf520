<template>
    ...
    <div>
        ...
        <el-button type="text" @click="audit" v-if="scope.row.audit_status != 1">审核</el-button>
        <template>
            <el-dropdown trigger="click" @command="(command) => {fastAudit(scope, command)}" v-if="scope.row.audit_status != 1">
                <el-button type="text">
                    快捷审核 <i class="el-icon-arrow-down"></i>
                </el-button>
                <el-dropdown-menu slot="dropdown">
                    <el-dropdown-item command="1" v-if="scope.row.audit_status == 3">审核通过</el-dropdown-item>
                    <el-dropdown-item command="2" v-if="scope.row.audit_status == 2">审核不通过</el-dropdown-item>
                </el-dropdown-menu>
            </el-dropdown>
        </template>
