    ...
    <el-col>
        <el-form v-model="form" inline size="small">
            ...
            <el-form-item prop="goodsName" label="商品名">
                <el-input v-model="form.goodsName" clearable placeholder="请输入商品名"/>
            </el-form-item>
            <el-form-item prop="goodsType" label="商品类型">
                <el-select v-model="form.goodsType" placeholder="请选择" clearable class="w-100">
                    <el-option label="全部" :value="0"></el-option>
                    <el-option label="超市" :value="4"></el-option>
                </el-select>
            </el-form-item>
            <el-form-item prop="merchantNameOrId" label="商户">
                <el-input v-model="form.merchantNameOrId" clearable placeholder="请输入商户名或ID"/>
            </el-form-item>
        ...
        <el-table :data="list" v-loading="tableLoading" stripe @sort-change="sortChange">
            ...
            <el-table-column prop="cs_goods.goods_name" label="商品名"></el-table-column>
            <el-table-column prop="goods_type" label="商品类型">
                <template slot-scope="scope">
                    <span v-if="scope.row.goods_type == 1">团购</span>
                    <span v-else-if="scope.row.goods_type == 3">单品</span>
                    <span v-else-if="scope.row.goods_type == 4">超市</span>
                    <span v-else>未知({{scope.row.goods_type}})</span>
                </template>
            </el-table-column>
            <el-table-column label="商户">
                <template slot-scope="scope">
                    <span>[{{scope.row.cs_merchant.id}}]{{scope.row.cs_merchant.name}}</span>
                </template>
            </el-table-column>
            ...
            <el-table-column prop="pay_price" label="销售价"></el-table-column>
    ...
    export default {
        ...
        methods: {
            ...
            getList() {
                ...
                api.get('cs/act_hot/getStaGoodsList', this.form).then(data => {
                    this.list = data.list;
            ...
            exportExcel() {
                ...
                location.href = `/api/admin/cs/act_hot/exportStaGoods?${uri}`;
            },
        },
        created() {
            this.getList();
