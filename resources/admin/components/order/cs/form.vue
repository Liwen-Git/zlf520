    ...
    <div>
        <el-form label-width="150px">
            ...
            <el-form-item label="历时：">
                <span v-if="order.take_time && order.take_time.split('时')[0] >= 24 && order.status == 8">
            ...
            <el-form-item label="备注：">
                <span style="color: red;">{{order.remark}}</span>
            </el-form-item>
            ...
            <el-form-item label="提货时间：" v-if="order.deliver_type == 2 && order.status == 7">
            ...
            <el-form-item label="商家秀推荐人：">
                <span v-if="order.ybk_anchor">
                    {{order.ybk_anchor.nickname}}
                </span>
            </el-form-item>
            <el-form-item label-width="50px">
                <span v-if="order.status == 7 && order.auto_take_deliver == 1">用户超7天未确认收货，系统自动确认收货</span>
            </el-form-item>
        </el-form>
    </div>
</template>
    ...
    export default {
        ...
        methods:{
            initOrder(){
                ...
                this.order.statusText = ['','未支付', '已取消', '已关闭[超时自动关闭]', '已支付', '退款中[保留状态]', '已退款', '已完成[不可退款]', '待发货', '待自提', '已发货', '已付款', '退款失败'][parseInt(this.order.status)];
