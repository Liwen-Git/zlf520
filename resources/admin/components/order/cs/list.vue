        ...
        <el-form inline :model="query" size="small">
            ...
            <el-form-item label="订单状态" v-if="activeTab == 'all'">
                <el-select v-model="query.status" class="w-100" clearable>
                    ...
                    <el-option label="已付款" value="11"/>
                    <el-option label="退款失败" value="12"/>
                </el-select>
            </el-form-item>

            <el-form-item label="活动类型">
                <el-select v-model="query.marketingId" class="w-100" clearable>
                    <el-option label="全部" value=""/>
        ...
        <el-table :data="list" stripe>
            ...
            <el-table-column prop="take_time" label="历时">
                <template slot-scope="scope">
                    <span v-if="scope.row.take_time && scope.row.take_time.split('时')[0] >= 24 && scope.row.status == 8">
            ...
            <el-table-column prop="status" label="订单状态">
                <template slot-scope="scope">
                    ...
                    <span v-else-if="parseInt(scope.row.status) === 11">已付款</span>
                    <span v-else-if="parseInt(scope.row.status) === 12" class="c-danger">退款失败</span>
                    <span v-else>未知 ({{scope.row.status}})</span>
                </template>
            </el-table-column>
            <el-table-column prop="marketing_id" label="活动类型">
                <template slot-scope="scope">
                    <span v-if="parseInt(scope.row.marketing_id) === 0">无</span>
                    <span v-else-if="parseInt(scope.row.marketing_id) === 1">限时抢购</span>
                    <span v-else-if="parseInt(scope.row.marketing_id) === 2">拼团</span>
                    <span v-else>未知 ({{scope.row.marketing_id}})</span>
                </template>
            </el-table-column>
            ...
            <el-table-column label="操作">
                <template slot-scope="scope">
                    ...
                    <el-button v-if="scope.row.status == 12" type="text" @click="refundMoney(scope.row)">重新退款</el-button>
                </template>
            </el-table-column>
        </el-table>
    ...
    export default {
        ...
        methods: {
            exportExcel(){
                ...
                location.href = '/api/admin/order/cs/export?' + array.join('&');
            },
            search(){
                this.query.page = 1;
                this.getList();
            ...
            getMerchants(query){
                if (query !== '') {
                    ...
                    api.get('cs/merchant/getCsMerchantName', {keyword: query}).then(data => {
                        this.merchants = data;
            ...
            refundMoney(row){
                api.get('manualRefund', {order_id: row.id}).then(data => {
                    this.getList();
                })
            }
        },
