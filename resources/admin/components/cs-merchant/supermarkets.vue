                ...
                <template slot-scope="scope">
                    ...
                    <el-dropdown class="m-l-10" trigger="click" @command="(command) => {audit(scope, command)}">
                        ...
                        <el-dropdown-menu slot="dropdown">
                            <el-dropdown-item command="1" v-if="parseInt(scope.row.audit_status)===2">审核通过</el-dropdown-item>
                            <el-dropdown-item command="2" v-if="parseInt(scope.row.audit_status)===1">审核不通过</el-dropdown-item>
                            <el-dropdown-item command="3">查看商户审核记录</el-dropdown-item>
                            <el-dropdown-item command="4">查看商品</el-dropdown-item>
                        </el-dropdown-menu>
                    </el-dropdown>
                </template>
            </el-table-column>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            audit(scope, type) {
                if (type == 3) {
                    this.$menu.change('/cs/merchant/unaudits', {name: scope.row.name})
                } else if (type == 4) {

                    this.$menu.change('/cs/goods', {merchantName: scope.row.name})
                } else {
            ...
            doAudit(){
                ...
                this.$confirm(`确定将商户 ${this.currentAuditRecord.name} ${auditTypeMessage}吗?`).then(() => {
                    ...
                    api.post('/cs/merchant/reAudit', reqData).then(data => {
